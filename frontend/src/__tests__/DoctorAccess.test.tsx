...
const decryptKeyWithWK = vi.fn(async () => ({}) as CryptoKey)
...
vi.mock('@/lib/ecies', () => ({
  decryptKeyWithWK: (...a: unknown[]) => decryptKeyWithWK(...(a as [])),
  base64ToWk: () => new Uint8Array(32),
}))
...
beforeEach(() => {
  vi.clearAllMocks()
  getEncryptedKey.mockResolvedValue(new Uint8Array(60))
...
describe('DoctorAccess', () => {
  it('denies access when the token does not exist', async () => {
    ...
    render(<DoctorAccess tokenId="t1" doctorPublicKey={DOCTOR} encryptionKey={null} />)
  ...
  it('lets the doctor paste the key manually and decrypts the record', async () => {
    getTokenInfo.mockResolvedValue({ documentId: 'doc1', patient: PATIENT, doctor: DOCTOR })
    verifyAccess.mockResolvedValue(true)
    const user = userEvent.setup()
    render(<DoctorAccess tokenId="t1" doctorPublicKey={DOCTOR} encryptionKey={null} />)

    const input = await screen.findByPlaceholderText(/Base64 key/i)
    await user.type(input, 'someWrappingKey')
    await user.click(screen.getByRole('button', { name: /Decrypt record/i }))
  ...
  it('shows an error with retry when decryption fails', async () => {
    ...
    render(<DoctorAccess tokenId="t1" doctorPublicKey={DOCTOR} encryptionKey="wk" />)
